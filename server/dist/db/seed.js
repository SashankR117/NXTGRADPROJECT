...
// ─── Source Types ─────────────────────────────────────────────
const SOURCES = ['appstore', 'playstore', 'reddit', 'forum', 'twitter', 'reviews', 'quickcommerce'];
...
export async function seed() {
    ...
    THEMES.forEach((theme, ti) => {
        ...
        for (let i = 0; i < targetDocs; i++) {
            ...
            db.run(`INSERT INTO documents (id, source, source_id, content, title, rating, author, author_meta, language, category, product, platform, sentiment_valence, sentiment_label, sentiment_confidence, topic_id, theme_id, created_at, ingested_at, fingerprint)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [docId, source, `${source}_${uuid().slice(0, 8)}`, content, title, ratingValue,
            ...
            for (const aspect of template.aspects) {
                db.run('INSERT INTO aspects (id, document_id, aspect_name, sentiment, snippet, confidence) VALUES (?, ?, ?, ?, ?, ?)', [uuid(), docId, aspect.name, aspect.sentiment, aspect.snippet, rand(0.7, 0.95)]);
                aspectCount++;
            }
        }
    });
    ...
    for (const ins of insightDefs) {
        ...
        db.run(`INSERT INTO insights (id, theme_id, insight_text, recommendation, confidence, actionability, user_segments, strategic_question, validation_status, source_types, evidence_count, generated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [insightId, themeIds[ins.themeIdx], ins.insight, ins.recommendation, ins.confidence, ins.actionability,
        ...
        for (const docId of evidenceDocs) {
            ...
            if (doc) {
                db.run('INSERT INTO evidence (id, insight_id, document_id, quote, source) VALUES (?, ?, ?, ?, ?)', [uuid(), insightId, docId, doc.content.slice(0, 200), doc.source]);
            }
        }
        insightCount++;
    }
    ...
    for (const source of SOURCES) {
        ...
        db.run('INSERT INTO pipeline_runs (id, source, status, documents_fetched, documents_processed, errors, started_at, completed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [uuid(), source, 'completed', fetched, fetched - Math.floor(rand(0, 5)), Math.floor(rand(0, 3)), randomDate(1), randomDate(0)]);
    }
