...
chatRouter.post('/', async (req, res) => {
    try {
        ...
        execute('INSERT INTO chat_history (id, session_id, role, content, citations) VALUES (?, ?, ?, ?, ?)', [uuid(), sessionId, 'assistant', response.content, JSON.stringify(response.citations)]);
        res.json({
            sessionId,
...
chatRouter.get('/history', (req, res) => {
    try {
        ...
        res.json({ messages: queryAll('SELECT * FROM chat_history WHERE session_id = ? ORDER BY created_at ASC', [sessionId]) });
    }
    catch (err) {
...
async function generateResponse(query, context) {
    ...
    if (geminiApiKey && geminiApiKey.trim().length > 5) {
        try {
            return await generateGeminiResponse(query, context, geminiApiKey.trim());
        }
        catch (e) {
            console.error('Gemini API call failed:', e);
            return {
                content: `⚠️ **Gemini API Error**: ${e.message || 'Failed to call Gemini API'}\n\nPlease check your \`GEMINI_API_KEY\` in your \`.env\` file or verify API quota details.`,
                citations: []
            };
        }
    }
    const claudeApiKey = process.env.ANTHROPIC_API_KEY;
    if (claudeApiKey && claudeApiKey.trim().length > 5) {
        try {
            return await generateClaudeResponse(query, context, claudeApiKey.trim());
        }
        catch (e) {
...
async function generateGeminiResponse(query, context, apiKey) {
...
## Context - Themes:
${context.themes.map((t) => `- **${t.name}** (${t.document_count} docs, avg sentiment: ${(t.avg_sentiment || 0).toFixed(2)}): ${t.description}`).join('\n')}

## Context - Strategic Insights:
${context.insights.map((i) => `- [${i.theme_name}] ${i.insight_text} (Confidence: ${(i.confidence * 100).toFixed(0)}%)`).join('\n')}

## Context - Relevant Feedback Quotes:
${context.documents.slice(0, 15).map((d) => `- [Source: ${d.source}, ${d.created_at?.split('T')[0]}] "${d.content.slice(0, 200)}"`).join('\n')}

## Context - Top Mentions/Aspects:
${context.aspects.map((a) => `- ${a.aspect_name} (${a.sentiment}): ${a.count} mentions`).join('\n')}`;
...
async function generateClaudeResponse(query, context, apiKey) {
...
## Themes:
${context.themes.map((t) => `- **${t.name}** (${t.document_count} docs, sentiment: ${(t.avg_sentiment || 0).toFixed(2)}): ${t.description}`).join('\n')}

## Key Insights:
${context.insights.map((i) => `- [${i.theme_name}] ${i.insight_text} (${(i.confidence * 100).toFixed(0)}%)`).join('\n')}
...
function generateLocalResponse(query, context) {
    ...
    if (bestMatch >= 0 && context.insights[questionMap[bestMatch].idx]) {
        const insight = context.insights[questionMap[bestMatch].idx];
        const relevantDocs = context.documents.filter((d) => d.theme_name === insight.theme_name).slice(0, 5);
        if (relevantDocs.length === 0) {
            relevantDocs.push(...context.documents.slice(0, 3));
        }
    ...
    r += `### Top Themes\n`;
    for (const t of context.themes.slice(0, 5))
        r += `- **${t.name}** — ${t.document_count} docs (Sentiment: ${(t.avg_sentiment || 0).toFixed(2)})\n`;
    r += `\n### Key Insights\n`;
    for (const i of context.insights.slice(0, 3))
        r += `- ${i.insight_text.slice(0, 200)}... (${(i.confidence * 100).toFixed(0)}%)\n`;
    if (context.documents.length > 0) {
        r += `\n### Relevant Quotes\n`;
        for (const d of context.documents.slice(0, 3)) {
